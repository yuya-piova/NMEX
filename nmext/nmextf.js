...
(function() {
  ...
  FUNCTION_T.yotei_list = {};
  FUNCTION_T.yotei_list.popmenu = function() {
    popmenu.appendItems([{ text: 'GAS登録改', handler: () => syncToGoogleCalendar() }]);
    /**
     * 設定・定数
     */
    ...
    class ScheduleParser {
      constructor(tableElement) {
        ...
        // 基準年を取得（入力フィールドinput1_dtから取得、なければ現在年）
      ...
      _parseRow(row) {
        ...
        // ヘッダーマップを使って安全にセルを取得
        const dateCell = cells[this.headers['日付']];
        const timeCell = cells[this.headers['時間']];
        const titleCell = cells[this.headers['予定']];
        const placeCell = cells[this.headers['場所']];
        const contentCell = cells[this.headers['内容']];

        if (!dateCell || !timeCell || !titleCell) return null;

        const dateText = dateCell.innerText.trim(); // 例: "12/01"
        const timeText = timeCell.innerText.trim(); // 例: "10:00-11:00"

        if (!dateText || !timeText) return null;

        // 時間文字列の分割 ("10:00-11:00")
        // 全角ハイフンなどが混じる場合も考慮して数値とコロン以外で分割などを検討しても良いが、
        // ここでは元のロジック通り位置指定(substr)またはハイフン分割で行う
        // 時間フォーマットが固定(HH:mm-HH:mm)なら substr が最速
        const startTimeStr = timeText.substring(0, 5); // "10:00"
        const endTimeStr = timeText.substring(6, 11); // "11:00"

        // Dateオブジェクトの生成
        const startTime = this._createDate(dateText, startTimeStr);
        const endTime = this._createDate(dateText, endTimeStr);

        // 終了時間が開始時間より前の場合（日またぎ）
        // 例: 23:00 - 01:00
        ...
        let location = placeCell.innerText.trim();
        let description = ''; // 備考などがあれば取得
        ...
        switch (cleanTitle) {
          case '指導':
            ...
            break;
          case '会議・研修':
            title = `会議研修：${cleanTitle}`;
            break;
          case 'タスク：':
            title = `タスク：${cleanTitle}`; // 必要なら後ろの文字列を結合
            break;
          default:
            title = cleanTitle;
            break;
        ...
        return {
          ...
          colorId: this._getColorId(title),
      ...
      _createDate(dateStr, timeStr) {
        // dateStr: "12/01", timeStr: "10:00"
        // "/" を "-" に置換して標準フォーマットに近づける
        const formattedDate = dateStr.replace(/\//g, '-');
        // ISOフォーマット (YYYY-MM-DDTHH:mm:00) を作成
        // ※月日が1桁の場合のパディングが必要なら追加するが、
        // new Date("2024/1/1 10:00") は多くのブラウザで動作する
        const dateTimeStr = `${this.baseYear}/${formattedDate} ${timeStr}:00`;

        const date = new Date(dateTimeStr);

        // もし生成された日付が無効なら（万が一パース失敗時）
       ...
       */
      _getColorId(title) {
        if (CALENDAR_CONFIG.COLOR_MAP[title]) {
          return CALENDAR_CONFIG.COLOR_MAP[title];
        }
        if (title.indexOf('タスク：') !== -1) {
          return CALENDAR_CONFIG.TASK_COLOR;
        }
        return CALENDAR_CONFIG.DEFAULT_COLOR;
      }
    }

    /**
     * ユーティリティ: 日付フォーマッター (YYYY/MM/DD)
     */
    ...
    async function syncToGoogleCalendar() {
      ...
      // カレンダー設定の取得
      const calendarId = typeof myprofiles !== 'undefined' ? myprofiles.getone({ calendar_id: false }) : '';
      const scriptUrl = NX.ENDPOINT.makeGCal;
      //typeof myprofiles !== 'undefined' ? myprofiles.getone({ calendar_url: null }) : '';

      if (!scriptUrl) {
        alert('カレンダーURL設定がありません');
        return;
      }

      // 日付範囲の取得
      const startDate = new Date(inputDt1.value);
      const endDateRaw = new Date(inputDt2.value);
      const endDate = new Date(endDateRaw);
      endDate.setDate(endDate.getDate() + 1);

      const rangeStartStr = formatDateSlash(startDate);
      const rangeEndStr = formatDateSlash(endDate);

      // 2. データの分割 (バッチ処理)
      const BATCH_SIZE = 30; // 30件ずつ送信 (安全マージン)
      ...
      msgDiv.style.background = '#fff3cd'; // 処理中は黄色
      msgDiv.innerText = `送信準備中... (全 ${allEvents.length} 件)`;

      try {
        for (let i = 0; i < totalBatches; i++) {
          const startIdx = i * BATCH_SIZE;
          const endIdx = startIdx + BATCH_SIZE;
          const chunk = allEvents.slice(startIdx, endIdx);

          // 最初のバッチのみ「cleanup (削除)」フラグを立てる
          const isFirstBatch = i === 0;

          msgDiv.innerText = `送信中... ${i + 1} / ${totalBatches} セット目を処理しています`;
        ...
        // 完了表示
        msgDiv.style.background = '#d4edda'; // 緑色
        msgDiv.innerText = `完了: 全 ${allEvents.length} 件のイベントを同期しました`;
      } catch (error) {
        console.error('送信エラー:', error);
        msgDiv.style.background = '#f8d7da'; // 赤色
        msgDiv.innerText = `エラーが発生しました: ${error.message}`;
      }
    }
        ...
        on: {
          click: () => {
            ...
            $table.find('tr:gt(0)').each(function() {
              const $tr = $(this);
              let [dt, summary, location, description] = $tr.findTdToArray(
                tableHead['日付'],
                tableHead['予定'],
                tableHead['場所'],
                tableHead['内容']
