...
function getUniqueSelector(element) {
  ...
  if (element.className && typeof element.className === 'string') {
    const cls = element.className.split(/\s+/)[0];
    if (cls) return `.${cls}`;
  }
...
class ExDate extends Date {
   ...
   * @param {string} format - yyyy/mm/dd aaaa HH:MM
  ...
  as(format) {
    ...
    const values = {
      ...
      yy: this.getFullYear()
        ...
        .slice(-2),
      m: this.getMonth() + 1,
      mm: this.padWithZero(this.getMonth() + 1, 2),
      d: this.getDate(),
      dd: this.padWithZero(this.getDate(), 2),
      H: this.getHours(),
      HH: this.padWithZero(this.getHours(), 2),
      M: this.getMinutes(),
      MM: this.padWithZero(this.getMinutes(), 2),
      S: this.getSeconds(),
      SS: this.padWithZero(this.getSeconds(), 2),
      aaaa: this.DATE['aaaa'][this.getDay()],
      aaa: this.DATE['aaa'][this.getDay()],
      aa: this.DATE['aa'][this.getDay()],
      a: this.DATE['a'][this.getDay()]
    };
    return format.replace(/yyyy|yy|m{1,2}|d{1,2}|H{1,2}|M{1,2}|S{1,2}|a{1,4}/g, match => values[match]);
  }
  /**
   * afteryears 指定年数後（前）を取得
   * @param {number} years [前であれば負値]
   * @return {object} dateobj
   * @memberof ExDate
   */
  afteryears(years) {
    if (typeof years == 'string') years = parseInt(years);
    this.setFullYear(this.getFullYear() + years);
    return this;
  }
  /**
   * aftermonths 指定月数後（前）を取得
   * @param {number} months [前であれば負値]
   * @return {object} dateobj
   * @memberof ExDate
   */
  aftermonths(months) {
    if (typeof months == 'string') months = parseInt(months);
    this.setMonth(this.getMonth() + months);
    return this;
  }
  /**
   * afterdays 指定日数後（前）を取得
   * @param {number} days [前であれば負値]
   * @return {object} dateobj
   * @memberof ExDate
   */
  afterdays(days) {
  ...
  nextday(daystr) {
    ...
    const targetnumber = Math.max(
      ...Object.values(this.DATE).map(arr => arr.findIndex(elm => elm === daystr))
    );
    if (targetnumber == -1) {
      console.warn('ExDate', daystr, '曜日が見つかりませんでした');
      return false;
    }
  ...
  endofmonth() {
    this.setDateTry(null, null, 0).aftermonths(1);
    return this;
  }
  /**
  ...
  #getWarekiData() {
    ...
    for (var i = 1800; i < 3000; i++) {
      ...
      if (dtstring.indexOf('1/') != -1) rtn[dtstring.match(/.*(?=1\/)/)] = i;
    }
    return rtn;
  }
}
